...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict,
    ...
    with torch.no_grad():
        for i in range(0, len(all_sens), batch_size):
            # print(i, len(all_sens), batch_size)
    ...
    # exit(0)
    return total_embedding.detach().cpu(), lens, mask, padded_idf, tokens
...
def word_mover_score(refs, hyps, idf_dict_ref, idf_dict_hyp, stop_words=[], n_gram=1, remove_subwords = True, batch_size=1024):
    preds = []
