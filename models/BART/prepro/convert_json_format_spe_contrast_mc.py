...
rouge = Rouge()
org_data_path = 'data/mc/'
new_data_path = 'json_spe_contrast/mc/'


def preprocess_summary(summary):
    summary = summary.replace("针对本次问诊，医生更新了总结建议:", "")
    summary = summary.replace("针对本次问诊，医生更新了总结建议：", "")
    summary = summary.replace("针对本次问诊,医生更新了总结建议:", "")
    summary = summary.replace("针对本次问诊,医生更新了总结建议：", "")
    summary = summary.replace("针对本次问诊,医生更新了总结建议。", "")
    summary = summary.replace("针对本次问诊,医生更新了总结建议.", "")
    summary = summary.replace("医生更新了总结建议:", "")
    summary = summary.replace("医生更新了总结建议。", "")
    summary= summary.replace("医生有语音回复内容，具体请点击收听医嘱。", "")
    summary = summary.replace("医生有语音回复内容，具体请点击收听医嘱.", "")
    summary = summary.replace("医生有语音回复内容,具体请点击收听医嘱.", "")
    summary = summary.replace("医生有语音回复内容,具体请点击收听医嘱。", "")
    summary = summary.replace("医生有语音回复内容,具体请点击收听医嘱，", "")
    ...
    summary = summary.replace('!', '！')
    summary = summary.replace(':', '：')
    if '（男' in summary or '（女' in summary:
        id = summary.find('）')
        if id != -1:
...
def convert(data, is_train=True):
    ...
    for i in range(3):
        for sample in tqdm(data):
            ...
            tmp_utt_id = []
            turn_num = 1
            for j, turn in enumerate(sample["content"]):
                utterance = turn["utterance"].strip().replace(' ', '')
                utterance = utterance.replace("针对本次问诊，医生更新了总结建议：", "")
                utterance = utterance.replace("针对本次问诊，医生更新了总结建议:", "")
                utterance = utterance.replace("针对本次问诊,医生更新了总结建议：", "")
                utterance = utterance.replace("针对本次问诊,医生更新了总结建议:", "")
                utterance = utterance.replace("针对本次问诊,医生更新了总结建议。", "")
                utterance = utterance.replace("针对本次问诊,医生更新了总结建议.", "")
                utterance = utterance.replace("医生更新了总结建议:", "")
                utterance = utterance.replace("医生更新了总结建议：", "")
                utterance = utterance.replace("医生更新了总结建议。", "")
                utterance = utterance.replace("图片因隐私问题无法显示", "")
                utterance = utterance.replace("语音因隐私问题无法显示", "")
                utterance = utterance.replace("医生有语音回复内容，具体请点击收听医嘱。", "")
                utterance = utterance.replace("医生有语音回复内容，具体请点击收听医嘱.", "")
                utterance = utterance.replace("医生有语音回复内容,具体请点击收听医嘱.", "")
                utterance = utterance.replace("医生有语音回复内容,具体请点击收听医嘱。", "")
                utterance = utterance.replace("医生有语音回复内容,具体请点击收听医嘱，", "")
                ...
                utterance = utterance.replace('!', '！')
                utterance = utterance.replace(':', '：')
                if '（男' in utterance or '（女' in utterance:
                    id = utterance.find('）')
                    if id != -1:
                        utterance = utterance[:(id + 1)]
                ...
                if turn["speaker"] != previous_speaker:
                    if tmp_utt != []:
                        ...
                        if turn["speaker"] == "患者":
                            utter['type'] = 'agent'
                            utter['content'] = "医生：" + ''.join(tmp_utt)
                        else:
                            utter['type'] = 'user'
                            utter['content'] = "患者：" + ''.join(tmp_utt)
            ...
            if i == 0:
                user_sum = preprocess_summary(sample["summary"]["description"])
                new_sample['summary'] = [user_sum]
                if len(user_sum) < 2:
                    continue
            elif i == 1:
                agent_sum = preprocess_summary(sample["summary"]["suggestion"])
                ...
                user_sum = preprocess_summary(sample["summary"]["description"])
                agent_sum = preprocess_summary(sample["summary"]["suggestion"])
                new_sample['summary'] = [user_sum, agent_sum]
                if len(user_sum) < 2:
                    continue
                if len(agent_sum) < 2:
                    continue
...
if __name__ == '__main__':
    ...
    for name in ['train', 'val', 'test']:
        ...
        with open(new_data_path + 'all' + '/' + 'mc.' + name + '.json', 'w', encoding="utf-8") as f:
            json.dump(new_data, f, indent=4, ensure_ascii=False)
        with open(new_data_path + 'user' + '/' + 'mc.' + name + '.json', 'w', encoding="utf-8") as f:
            json.dump(new_data_user, f, indent=4, ensure_ascii=False)
        with open(new_data_path + 'agent' + '/' + 'mc.' + name + '.json', 'w', encoding="utf-8") as f:
            json.dump(new_data_agent, f, indent=4, ensure_ascii=False)
        with open(new_data_path + 'final' + '/' + 'mc.' + name + '.json', 'w', encoding="utf-8") as f:
