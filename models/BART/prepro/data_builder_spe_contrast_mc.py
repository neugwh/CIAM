...
class BertData():
    def __init__(self, args):
        ...
        self.eou_token = '[EOU]'
        self.eop_token = '[EOP]'
        self.user_spe_token='[U]'
        self.agent_spe_token='[A]'
        ...
        self.user_spe_vid = self.tokenizer.vocab[self.user_spe_token]
        self.agent_spe_vid = self.tokenizer.vocab[self.agent_spe_token]
    ...
    def preprocess_summary(self, summary_list, type):
        ...
        for summ in summary_list:
            ...
            if type == 0:
                content_subtokens.append(self.user_spe_token)
            elif type == 1:
                content_subtokens.append(self.agent_spe_token)
            ...
            if (id != len(summary_list)):
                content_subtokens.append(self.eop_token)
                utt_ids.extend([id] * (len(sub_tokens) + 2))
            else:
