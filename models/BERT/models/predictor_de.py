...
def build_predictor(args, tokenizer, symbols, model, mode,logger=None):
    scorer = GNMTGlobalScorer(args.alpha,length_penalty='wu')

    translator = Translator(args, model, tokenizer, symbols, mode=mode,global_scorer=scorer,logger=logger)
    return translator
...
class Translator(object):
    ...
    def from_batch(self, translation_batch):
        ...
        preds, pred_score, gold_score, tgt_txt, src =  translation_batch["predictions"],translation_batch["scores"],translation_batch["gold_score"],batch.tgt_txt, batch.src

        translations = []
        for b in range(batch_size):
            pred_sents = self.vocab.convert_ids_to_tokens([int(n) for n in preds[b][0]])
            pred_sents = ''.join(pred_sents).replace(' ##','')
            gold_sent = ''.join(tgt_txt[b].split())
    ...
    def translate(self,
                  data_iter, save_path, step,mode,
                  attn_debug=False):

        self.model.eval()
        result_path = os.path.join(self.args.result_path, save_path)
        if not os.path.exists(result_path):
            os.makedirs(result_path)
        gold_path = os.path.join(result_path, '.%d.gold' % step)
        can_path = os.path.join(result_path, '.%d.candidate' % step)
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold = trans
                    pred_str = pred.replace('[BOS]', '').replace('[EOS]', '').replace('[PAD]', ''). \
                        replace("[USERSUM]", "").replace("[AGENTSUM]", "").replace("[FINALSUM]", ""). \
                        replace('[EOU]', '').replace('[EOP]', '').replace(' ', '').replace('[U]', '').replace('[A]','').replace(' ', '').strip()
                    gold_str = gold.strip()
        ...
        calculate_zh(result_path, can_path, gold_path,mode)
    ...
    def _fast_translate_batch(self,
        ...
        src_features = self.model.bert(src, seg_ids, mask_src)
        ...
        for step in range(max_length):
            ...
            alive_seq = torch.cat(
                [alive_seq.to(torch.long).index_select(0, select_indices.to(torch.long)),
