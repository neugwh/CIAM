...
class BertData():
    def __init__(self, args):
        ...
        self.eos_vid = self.tokenizer.vocab[self.eos_token]
    ...
    def preprocess_summary(self, summary_list, type):
        ...
        if type == 0:
            tgt = [self.user_bos_vid] + self.tokenizer.convert_tokens_to_ids(content_subtokens)+[self.eos_vid]
            start_token = [self.user_bos_vid]
        elif type == 1:
            tgt = [self.agent_bos_vid] + self.tokenizer.convert_tokens_to_ids(content_subtokens)+[self.eos_vid]
            start_token = [self.agent_bos_vid]
        elif type == 2:
            tgt = [self.final_bos_vid] + self.tokenizer.convert_tokens_to_ids(content_subtokens)+[self.eos_vid]
            start_token = [self.final_bos_vid]


        utt_ids = [utt_ids[0]] + utt_ids+[utt_ids[-1]]
        assert len(tgt) == len(utt_ids)
        return tgt, start_token, original_txt, utt_ids
...
def _format_to_bert(params):
    ...
    for dialogue in jobs:
        ...
        tgt, satrt_token, original_txt, utt_ids = summ_b_data
        b_data_dict = {"tgt": tgt, "start_token": satrt_token,
